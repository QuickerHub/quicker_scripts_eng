...
namespace QuickerChromeController
{
    public class ChromeBridge
    {
        ...
        private readonly Func<string, object, bool, int, BrowserRespMessage<JToken>> _runBackgroundCommandFunc;
        ...
        /// 初始化Chrome桥接
        /// </summary>
        public ChromeBridge(ScriptEngineWrapper scriptEngine, Func<string, object, bool, int, BrowserRespMessage<JToken>> runBackgroundCommandFunc, int timeoutMs, CancellationToken? cancellationToken)
        {
            _scriptEngine = scriptEngine ?? throw new ArgumentNullException(nameof(scriptEngine));
            _runBackgroundCommandFunc = runBackgroundCommandFunc;
            _timeoutMs = timeoutMs;
            _cancellationToken = cancellationToken;

            // 注入简单的API调用函数，返回结果由JavaScript端处理
            _scriptEngine.InjectFunction("_nativeApiCall", new Func<string, object, bool, JToken>((apiName, parameters, hasCallback) =>
            {
                try
                {
                    Debug.WriteLine($"[ChromeBridgeMVP] 收到API命令: {apiName} 参数:{parameters.ToJson()} hasCallBack:{hasCallback}");
                    var result = HandleApiCall(apiName, parameters, hasCallback);
                    Debug.WriteLine($"调用结果：{result?.ToJson()}");
                    return result;
                }
                catch (Exception ex)
                {
            ...
            _scriptEngine.InjectObject("log", new Action<string>(message =>
            {
                Debug.WriteLine($"[JS日志] {message}");
            }));


            // 从嵌入资源加载Chrome API脚本

            _scriptEngine.Execute(CHROME_API);
        }
        ...
        /// 处理API调用并返回结果
        /// </summary>
        private JToken HandleApiCall(string apiName, object parameters, bool hasCallback)
        {
            ...
            {
                // 如果有回调，则等待执行完成。
                var result = _runBackgroundCommandFunc(apiName, parameters, hasCallback, _timeoutMs);
                if (result != null)
                {
                    if (result.IsSuccess)
                    {
                    ...
                    return new JObject { ["error"] = result.Message };
                }
                else
                {
                    return new JObject();
            ...
            {
                Debug.WriteLine($"[ChromeBridgeMVP] 处理API时异常: {ex.Message}");
                return new JObject { ["error"] = ex.Message };
            }
        }
        ...
        public static string CHROME_API = /*lang=javascript*/
"""
// 创建chrome对象
